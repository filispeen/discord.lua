...
describe("Route parity endpoints", function()
    ...
    it("builds message, thread and webhook parity endpoints", function()
        ...
        route:get_archived_threads("c1", "joined_private", { before = "t1", limit = 20 })
        ...
        assert.is_truthy(calls[5].endpoint:find("/channels/c1/users/@me/threads/archived/private?", 1, true))
        assert.is_truthy(calls[5].endpoint:find("before=t1", 1, true))
        assert.is_truthy(calls[5].endpoint:find("limit=20", 1, true))
        assert.equals("/channels/c1/webhooks", calls[6].endpoint)
        assert.equals("/channels/c1/followers", calls[7].endpoint)
    end)
