...
describe("Client remaining gateway dispatch wiring", function()
    ...
    it("updates emoji and sticker model caches from guild snapshots", function()
        local client, handlers = make_client_with_fake_gateway()
        handlers["GUILD_EMOJIS_UPDATE"]({ guild_id = "guild1", emojis = { { id = "emoji1", name = "wave" } } })
        handlers["GUILD_STICKERS_UPDATE"]({ guild_id = "guild1", stickers = { { id = "sticker1", name = "hello" } } })

        assert.equals("wave", client.emojis.guild1[1].name)
        assert.equals("guild1", client.emojis.guild1[1].guild_id)
        assert.equals("hello", client.stickers.guild1[1].name)
        assert.equals("guild1", client.stickers.guild1[1].guild_id)
    end)
end)
