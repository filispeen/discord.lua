...
-- Wrap a suspended coroutine and surface failures with its traceback.
...
-- Coordinates shutdown between the read and write halves of a uv stream.
...
local function makeWrite(socket, closer)
  ...
  local function wait()
    ...
    return function(callbackErr)
      if hasYielded then
        hasYielded = false
        if coroutine.status(thread) == "suspended" then
          assertResume(thread, callbackErr)
        end
      else
        cbErr = callbackErr
        hasReturned = true
