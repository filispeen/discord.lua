...
function SKU:iter_subscriptions(opts)
    opts = opts or {}
    if not self.http then
        error("SKU has no http client attached, cannot iterate subscriptions", 0)
    end
    ...
    return Paginator.new(function(page)
        ...
        local data = Route.new(self.http):get_sku_subscriptions(self.id, params)
        local result = {}
        for i, item in ipairs(data or {}) do
            result[i] = Subscription.new(item, self.http)
        end
        return result
    end, {
